...
What this does, end to end:
  ...
  6. writes the key file AND a ready-to-run wrapper script, so syncing the new
     library is a single command
...
def main():
    ...
    ap.add_argument("--watch-dir", default=None,
                    help="folder this library will sync from (used in the wrapper script)")
    ...
    ap.add_argument("--scripts-dir", default=".", help="where to write the wrapper script")
    ...
    if coll_id:
        ...
        warn("id from its URL, then put it in the wrapper script below.")
    ...
    conf = pathlib.Path(a.scripts_dir).expanduser() / f"{a.collection.lower()}.conf"
